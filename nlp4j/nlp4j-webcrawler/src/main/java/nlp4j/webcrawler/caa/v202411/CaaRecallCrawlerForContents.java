...
public class CaaRecallCrawlerForContents {
	static private Logger logger = LogManager.getLogger(MethodHandles.lookup().lookupClass());

	static public JsonObject crawl(String url) throws IOException {
		...
		try (HttpClient client = (new HttpClientBuilder()).build();) {
			...
			if (res.getResponseCode() == 200) {
				...
				{ // 対象の特定情報
					String att = "対象の特定情報";
					attrs.add(att);
					String text = contents.get(2);
					md.append("# " + att + "\n" + text + "\n");
				}
				{ // 参照情報
					// String selector = "#contents_inner > div.detail_box_bottom.clearfix > ul >
					// li:nth-child(2) > span.detail_text.clearfix.quilltext";
					// Elements els = jsDoc.select(selector);
					// System.err.println(els.text());
					// attrs.add("参照情報");
				}
				{ // 参照情報リンク
					...
					String att = "参照情報";
					attrs.add(att);
					String text = els.attr("href");
					md.append("# " + att + "\n" + text + "\n");
				}
				{ // 備考
					String att = "備考";
					attrs.add(att);
					String text = contents.get(3);
					md.append("# " + att + "\n" + text + "\n");
				}
