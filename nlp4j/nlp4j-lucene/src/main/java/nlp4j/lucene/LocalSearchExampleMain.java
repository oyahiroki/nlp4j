...
public class LocalSearchExampleMain {
	...
	public static void main(String[] args) {
		...
		try (LocalSearch search = new LocalSearch("en")) {
			search.add("1", "Developers are searching documents with a local search engine.");
			search.add("2", "A developer searched many documents yesterday.");
			search.add("3", "This tool searches local JSON documents.");
			search.add("4", "Lucene's EnglishAnalyzer is useful for English full-text search.");
			search.add("5", "The quick brown fox jumps over the lazy dog.");
			search.commit();
