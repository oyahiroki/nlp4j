...
#		self.nlp = spacy.load("ja_ginza")
	...
	def myProcess(self, t0,t1,t2,t3):
		...
		try:
			...
			query_embedding = self.model.encode(t0)
			corpus_embeddings = self.model.encode([t1, t2, t3])
...
#            print(self.headers)
			...
			# JSONデータを解析して辞書として取得
			try:
				parsed_data = json.loads(requestbody)
			...
			# 't1', 't2', 't3', ...のようにパラメータを取得して "hello" を追加
			response_data = {}
			for key in sorted(parsed_data.keys()):
				if key.startswith('t'):
					response_data[key] = parsed_data[key] + " hello"
			
			# JSON形式でレスポンスを返す
			response_json = json.dumps(response_data).encode('utf-8')			
			
			t0 = requestbody.get('t0')
			t1 = requestbody.get('t1')
			t2 = requestbody.get('t2')
			t3 = requestbody.get('t3')

#			self.myProcess(t0,t1,t2,t3)
			res = {}
			self.send_response(200)
			self.send_header("Content-type","application/json; charset=utf-8")
			self.end_headers()
			responsebody = json.dumps(response_json)
			# wfile Contains the output stream for writing a response back to the client. Proper adherence to the HTTP protocol must be used when writing to this stream in order to achieve successful interoperation with HTTP clients.
...
class HelloHttpServer(ThreadingMixIn, HTTPServer):
	def __init__(self, address, handlerClass=HelloHttpRequestHandler, option=0):
		...
		print("http://" + address[0] + ":" + str(address[1]) + "/?t0=これはテストです。&t1=これはテストです。$t2=これはテストです。&t3=これはテストです。")
		print("http://" + address[0] + ":" + str(address[1]) + "/?t0=%E3%81%93%E3%82%8C%E3%81%AF%E3%83%86%E3%82%B9%E3%83%88%E3%81%A7%E3%81%99%E3%80%82&t1=%E3%81%93%E3%82%8C%E3%81%AF%E3%83%86%E3%82%B9%E3%83%88%E3%81%A7%E3%81%99%E3%80%82&t2=%E3%81%93%E3%82%8C%E3%81%AF%E3%83%86%E3%82%B9%E3%83%88%E3%81%A7%E3%81%99%E3%80%82&t3=%E3%81%93%E3%82%8C%E3%81%AF%E3%83%86%E3%82%B9%E3%83%88%E3%81%A7%E3%81%99%E3%80%82")
