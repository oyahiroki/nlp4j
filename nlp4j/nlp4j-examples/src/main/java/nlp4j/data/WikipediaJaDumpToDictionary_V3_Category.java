...
public class WikipediaJaDumpToDictionary_V3_Category {
	...
	public static void main(String[] args) throws Exception {
		...
		String dir = "R:/usr/local/wiki/" + lang + media + "/20230101/";

		String dumpFileName = dir + lang + media + "-20230101-pages-articles-multistream.xml.bz2";

		String category_regex = ".*(漫画).*";
		String category_name = "漫画";

		String out_dicFileName = dir + lang + media + "-20230101-pages-dic_v3_" + category_name + ".txt";
		...
		WikiPageHandler wikiPageHander = new WikiPageHandler() {
			...
			public void read(WikiPage page) throws BreakException {
				if (page != null && page.getNamespace().equals("0")) {
					count++;
					if (debug && count > 10000) { // IF YOU WANT TO BREAK
					...
					String categories = page.getCategoryTags().toString();

					if (categories.matches(category_regex) == false) {
						return;
					}
					...
					if (entries.contains(title) == false) {
						pw.println(title);
						entries.add(title);
					}
